...
function PomodoroApp() {
  ...
  // --- Render ---
  return (
    <main className={`shell theme-${resolvedTheme} ${settings.theme.followSystem ? "theme-follow-system" : "theme-fixed"} ${hasCustomBackground ? "has-custom-background" : ""} ${activeBackgroundPalette ? "auto-matched-background" : ""}`} style={shellStyle}>
        ...
        <div className="settings-backdrop" role="presentation" onMouseDown={() => setSettingsOpen(false)}>
          <aside className="settings-panel" aria-label="设置" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <div className="settings-section core-settings-section">
              ...
              <p className="settings-note">项目会影响今日任务、菜单栏小树和森林统计；删除前会再次确认。</p>
            </div>
            ...
            {/* Menu bar */}
            <div className="settings-section"><span className="section-kicker">菜单栏</span>
              <SettingsToggle label="显示在菜单栏" checked={settings.menuBar.enabled} onChange={(checked) => patchSettings({ menuBar: { enabled: checked } })} />
              ...
              <p className="settings-note">位置由 macOS 管理，可按住 Command 拖动图标。</p>
            </div>
            ...
            <div className="settings-section"><span className="section-kicker">快捷键</span>
              ...
              <p className="settings-note">默认 Control + Option + P，可在后台直接开始或恢复番茄。{shortcutStatus ? ` ${shortcutStatus}` : ""}</p>
            </div>
...
function normalizeShortcut(event: Pick<KeyboardEvent, "key" | "metaKey" | "ctrlKey" | "altKey" | "shiftKey">) {
  ...
  if (!key) return "";
  const parts = [event.metaKey ? "Command" : "", event.ctrlKey ? "Ctrl" : "", event.altKey ? "Alt" : "", event.shiftKey ? "Shift" : "", key].filter(Boolean);
